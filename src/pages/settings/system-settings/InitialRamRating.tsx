...
export default function InitialRamRating() {
    const [search, searchVal, inputChange] = useDebounceSearch()
    const [isModalShow, setIsModalShow] = useState(false);
    const [selectedData, setSelectedData] = useState<TInitialRamRating | undefined>(undefined);
    const [loading, setLoading] = useState(true)
    const [dataSource, setDataSource] = useState<TInitialRamRating[]>([])
    ...
    async function fetchData(signal?: AbortSignal, params?: ApiParams) {
        setLoading(true)
        try {
            const res = await GET<ApiSuccess<TInitialRamRating[]>>('/initial_ram_ratings', signal!, params)
    ...
    const columns: ColumnsType<TInitialRamRating> = [
        {
            ...
            render: (_, record) => (
                <Space>
                    ...
                    <ButtonActions
                        ...
                        deleteData={() => DELETE('/initial_ram_ratings/' + record.id).finally((fetchData))}
    ...
    return (
        <>
            ...
            <Table<TInitialRamRating> loading={loading} columns={columns} dataSource={dataSource} isSizeChanger />
...
type ModalProps = {
    ...
    selectedData?: TInitialRamRating
}
...
function ModalInput({ open, onCancel, selectedData, fetchData }: ModalProps) {
    ...
    return <Modal open={open} onCancel={onCancel} footer={null} title={`Initial Ram Rating - ${selectedData ? 'Edit' : 'Create'}`} forceRender>
