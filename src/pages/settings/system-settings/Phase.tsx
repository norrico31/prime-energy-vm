...
export default function Phase() {
    ...
    const [selectedData, setSelectedData] = useState<TPhase | undefined>(undefined);
    const [loading, setLoading] = useState(true)
    const [dataSource, setDataSource] = useState<TPhase[]>([])
    ...
    async function fetchData(signal?: AbortSignal, params?: ApiParams) {
        setLoading(true)
        try {
            const res = await GET<ApiSuccess<TPhase[]>>('/phases', signal!, params)
    ...
    const columns: ColumnsType<TPhase> = [
        {
            title: 'Phase',
            dataIndex: 'name',
            key: 'name',
    ...
    return (
        <>
            ...
            <Table<TPhase> loading={loading} columns={columns} dataSource={dataSource} isSizeChanger />
...
function ModalInput({ open, onCancel, selectedData, fetchData }: ModalProps) {
    ...
    const onFinish = (v: Payload) => {
        setLoading(true)
        const result = !selectedData ? POST<Payload, ApiSuccess<TPhase>>('/phases/', v) : PUT<Payload, ApiSuccess<TPhase>>('/phases/' + selectedData.id, v);
        result.then(() => {
            setError(undefined)
            form.resetFields()
            onCancel()
    ...
    return <Modal open={open} onCancel={onCancel} footer={null} title={`Phase - ${selectedData ? 'Edit' : 'Create'}`} forceRender>
        <Form form={form} onFinish={onFinish} layout='vertical' disabled={loading}>
            ...
            <Form.Item label='Phase' name="name" rules={[{ required: true }]}>
                <Input type="text" placeholder="Enter phase name." />
            </Form.Item>
