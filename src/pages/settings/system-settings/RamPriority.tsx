...
function ModalInput({ open, onCancel, selectedData, fetchData }: ModalProps) {
    ...
    const onFinish = (v: Payload) => {
        setLoading(true)
        const result = !selectedData ? POST<Payload, ApiSuccess<TRamPriority>>('/ram_priority/', { ...v, is_active: v.is_active ? 1 : 0 }) : PUT<Payload>('/ram_priority/' + selectedData.id, { ...v, is_active: v.is_active ? 1 : 0 });
        result.then(() => {
            setError(undefined)
            form.resetFields()
            onCancel()
    ...
    return <Modal open={open} onCancel={onCancel} footer={null} title={`Ram Priority - ${selectedData ? 'Edit' : 'Create'}`} forceRender>
        <Form form={form} onFinish={onFinish} layout='vertical' disabled={loading}>
            ...
            <Form.Item label='Ram Priority Name' name="name" rules={[{ required: true, message: '' }]}>
